...
facility_level = 1
tier = 'tier1'
user_enter = 'Not Enter'
champion_status = 'Not Champion'
companies = ['Boblox Boxing Gloves', 'Big Hands, Big Fists',
             "Ethan's Writing Studio", 'Knockout Corp.',
             "Jackson's Fishing Gear", "Sproston Pub N' Ale!",
             "Fisher's Food Truck", "MS Photography", "Pea's Grocery Mart",
             "Gremblo's Model Kits"]
personality_effects = {
...
(B) "The champ looks strong...maybe just try to avoid his punches and wear him out?" """)
            ...
            if fight_outcome == 'win':
                print(
                    f"{fighter['first_name']} has defeated the champion and "
                    f"won! He is the boxing champion now!")
                fighter['win_record'] += 1
                print(
                    f"New record:  {fighter['win_record']} - {fighter['lose_record']}")
                update_reputation(fighter, random.randint(30, 65))
                champion_status = 'Champion!'
            elif fight_outcome == 'lose':
                print(
                    f"{fighter['first_name']} has been defeated by the "
                    f"champion and lost! It was still a valiant effort.")
                fighter['lose_record'] += 1
                print(
                    f"New record:  {fighter['win_record']} - {fighter['lose_record']}")
                update_reputation(fighter, random.randint(6, 15))
            print(_CHAPTER_LINE)
            print("FINAL STATS")
            print(f"Fighter Name: {fighter['name']}")
            print(f"Reputation : {fighter['reputation']}")
            print(f"Wins : {fighter['win_record']}")
            print(f"Losses : {fighter['lose_record']}")
            print(f"Champion Status : {champion_status}")
            print(f"Power : {fighter['power']}")
            print(f"Speed : {fighter['speed']}")
            print(f"Finesse : {fighter['finesse']}")
            print(f"Thank you so much for playing! Maybe shoot for an even "
                  f"better fighter next time?")
            print(_CHAPTER_LINE)
            if menu_input == 'C':
                print(_CHAPTER_LINE)
